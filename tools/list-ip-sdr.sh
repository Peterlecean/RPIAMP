...
do
  if [ -n "$client_ip" ]; then
    client_mp=$(curl --connect-timeout 3 --max-time 5 --silent http://$client_ip:8000/status-json.xsl | jq -r 'getpath(["icestats","source","server_name"])')
    if [ -n "$client_mp" ]; then
      ovnipssdr_alive+=$client_ip$'\n'
    fi
  fi
done < <(printf '%s\n' "$ovnips_alive")
echo "$ovnipssdr_alive"
